...
public class MultipleClientSingleThreadTest {
    ...
    public static void main(String[] args) {
        ...
        var loopGroup = NettyGroupBuilder.newBuilder().createPooledGroup(1);

        // Connecting 10 clients using only single thread. (1.20.4)
        for(int i=0;i<10;i++) {
            var client = new MinecraftClient(loopGroup);
            client.setBaseHandler(getHandler("LSProtocol" + i));
    ...
    private static IConnectionHandler getHandler(String username){
        return new IConnectionHandler() {
            ...
            public void active(Connection connection) {
                ...
                connection.sendPacket(new ClientLoginStartPacket(username, UUID.randomUUID()));
